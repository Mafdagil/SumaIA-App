...
st.markdown("""
    ...
    [data-testid="stSidebar"] .stMarkdown p, [data-testid="stSidebar"] label, [data-testid="stSidebar"] h2 {
    ...
    if arch_pdf:
        ...
        if filas:
            ...
            if st.button("💾 CERRAR MES Y GUARDAR HISTORIAL", use_container_width=True):
                conn = sqlite3.connect('sumaia_history.db')
                with conn: conn.execute("INSERT INTO cierres VALUES (?,?,?,?,?,?)", (datetime.now().strftime("%Y-%m-%d %H:%M"), ing, egr, com, nuevo_saldo, "CIERRE"))
                st.success("Cierre guardado correctamente.")
                st.rerun()
    else: st.info("Sube el PDF del banco para comenzar.")
